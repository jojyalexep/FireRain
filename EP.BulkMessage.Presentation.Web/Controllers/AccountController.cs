...
using EP.BulkMessage.Presentation.Web.Filters;
using EP.BulkMessage.Presentation.Web.Models;
using EP.BulkMessage.Presentation.Web.Modules;
using EP.BulkMessage.Service.Entity;
using EP.BulkMessage.Presentation.Web.Utilities;
...
namespace EP.BulkMessage.Presentation.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Manage(LocalPasswordModel model)
        {
            ...
            if (hasLocalAccount)
            {
                if (ModelState.IsValid)
                {
                    ...
                    model.NewPassword = model.NewPassword.ToLower();

                    // ChangePassword will throw an exception rather than return false in certain failure scenarios.
                    bool changePasswordSucceeded;
                    try
                    {
                        changePasswordSucceeded = UserModule.ChangePassword(User.Identity.Name, model.OldPassword, model.NewPassword);
                    }
                    catch (Exception ex)
                    {
        ...
        {
            if (UserModule.UserInRole(Role.Approver))
            {
                return RedirectToAction("Approval", "Campaign");
            }
            else
            {
